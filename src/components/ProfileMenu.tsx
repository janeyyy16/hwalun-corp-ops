  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 rounded-full border border-black/10 py-1.5 pl-1.5 pr-3 transition-colors hover:border-[var(--color-primary)]"
      >
        ...
        <div className="text-left">
          <p className="text-sm font-bold leading-tight text-[#1c2024]">{profile.full_name}</p>
      ...
      {open && (
        <div className="absolute right-0 top-full z-10 mt-2 w-56 overflow-hidden rounded-xl border border-black/5 bg-white py-1.5 shadow-lg">
          <Link
            to="/settings"
            onClick={() => setOpen(false)}
            className="flex items-center gap-2.5 px-4 py-2.5 text-sm font-semibold text-[#1c2024] hover:bg-black/5"
          >
