...
export async function getEntry(profileId: string, workDate: string): Promise<TimeEntry> {
  const { data, error } = await supabase
    .from("timecard_entries")
...
export async function approveTimecardCorrection(correction: TimecardCorrection): Promise<void> {
  ...
  const existing = await getEntry(correction.profileId, correction.workDate);
