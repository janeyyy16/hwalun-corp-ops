...
function MyTimecard() {
  ...
  async function handleSave(workDate: string, field: keyof TimeEntry, value: string) {
    const current = entries[workDate] ?? EMPTY_ENTRY;
    const updated = { ...current, [field]: value };
    setEntries((prev) => ({ ...prev, [workDate]: updated }));
    await saveEntry(profile!.id, workDate, updated);
  }
  ...
  return (
    <DashboardShell title="My Timecard" subtitle="Log your daily check-in and check-out times.">
            ...
            {days.map((day) => {
              ...
              return (
                <tr key={day} className="border-b border-black/5 last:border-0">
                  <td className="px-6 py-2 font-semibold text-[#1c2024]">{new Date(day).toLocaleDateString()}</td>
                  <td className="px-6 py-2">
                    <input
                      type="time"
                      value={entry.checkIn}
                      onChange={(e) => handleSave(day, "checkIn", e.target.value)}
                      className="rounded-lg border border-black/10 bg-white px-2 py-1.5 text-sm outline-none focus:border-[var(--color-primary)]"
                    />
                  </td>
                  <td className="px-6 py-2">
                    <input
                      ...
                      onChange={(e) => handleSave(day, "checkOut", e.target.value)}
                      className="rounded-lg border border-black/10 bg-white px-2 py-1.5 text-sm outline-none focus:border-[var(--color-primary)]"
                    />
                  </td>
                  <td className="px-6 py-2">
                    <input
                      type="time"
                      value={entry.mealStart}
                      onChange={(e) => handleSave(day, "mealStart", e.target.value)}
                      className="rounded-lg border border-black/10 bg-white px-2 py-1.5 text-sm outline-none focus:border-[var(--color-primary)]"
                    />
                  </td>
                  <td className="px-6 py-2">
                    <input
                      type="time"
                      value={entry.mealEnd}
                      onChange={(e) => handleSave(day, "mealEnd", e.target.value)}
                      className="rounded-lg border border-black/10 bg-white px-2 py-1.5 text-sm outline-none focus:border-[var(--color-primary)]"
                    />
                  </td>
                  <td className="px-6 py-2 text-[var(--color-steel)]">{calcWorkedHours(entry).toFixed(2)}</td>
                </tr>
              );
            })}
          </tbody>
